...
{
    // Formule de l'humidite absolue : https://en.wikipedia.org/wiki/Absolute_humidity#Calculating_absolute_humidity_from_relative_humidity_and_temperature
    float ah = 216.7f * ((h / 100.0f) * 6.112f *
                         exp((17.62f * t) / (243.12f + t)) / (273.15f + t));
    return (uint32_t)(ah * 256.0f); // Le SGP30 attend l'humidite absolue en format Q16.8 (valeur * 256)
}
...
void setup()
{
    ...
    pms.passiveMode(); // Le PMS5003 en mode passif permet de lire les donnees a la demande plutot que d'avoir des mises a jour toutes les secondes, ce qui est plus efficace pour notre usage ou on n'a pas besoin de mesures en temps reel
    pms.wakeUp();
...
void loop()
{
    ...
    if (now - tMqtt >= 10000)
    {
        ...
        if (mqtt.connected())
        {
            // Vider le buffer SD par lots (5 messages a la fois)

            /*
            512 KB = 512, 000 bytes ~400 bytes par message JSON

            512 000 / 400 = 1 480 messages max en buffer

            À 10s par message = ~3.5 heures d'autonomie sans MQTT
            */
...
void drawStatusBar()
{
    ...
    uint16_t sc = mqtt.connected() ? CLR_GOOD : WiFi.isConnected() ? CLR_MED
                                                                   : CLR_DANGER;
...
void drawPageEnv()
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (i < 3)
            M5.Lcd.drawFastHLine(10, ry + 38, 300, CLR_SEP);
        ry += 38;
    }
    M5.Lcd.setTextDatum(TL_DATUM);
}